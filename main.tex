...
Consider the example of a mug that is being warped using $\pcx{v} = \pcc + \mathrm{Reshape}(W v)$ (Equation \ref{eq:warp}). We can select any point $\pcx{v}_i$ and track it as the mug is being warped. For example, if the point lies on the handle of the mug, we can use it to align handles of mugs of different shapes and sizes. That can, in turn, facilitate the transfer of manipulation policies across mugs. The key question is how do we find the points $\pcx{v}_i$ relevant to a particular task.
...
\paragraph{Finding interaction points} For grasping, we define the interaction points as the pairs of contact points between the gripper and the object at the point of grasp. Let $\mathrm{pcd}^{(v)}$ and $\mathrm{obj}^{(v)}$
\begin{align}
    \{ p^{(v)}, p^{(G)} \} = \mathrm{FindContacts}(\mathrm{obj}^{(v)}, \mathrm{Transform}(\mathrm{grip}, T_{\mathrm{grasp}}))
\end{align}
